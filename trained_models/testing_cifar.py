...
from every_model_architecture import FFDNet_inspired_small_color  # Import the Small_cifar model class defined in an other file
...
##### LOAD THE MODEL #####
epoch = 10 # number of epoch of the already trained model
noise_std_train = 0.1 # amount of noise added during the training of the already trained model
model_path = f'trained_models/FFDNet_inspired_small_cifar_{epoch}_epochs_{noise_std_train}_noise_std.pth'
optimizer_path = f'trained_models/optimizer_FFDNet_inspired_small_cifar_{epoch}_epochs_{noise_std_train}_noise_std.pth'

small_cifar_model = FFDNet_inspired_small_color()  # Import class from every_model_architecture.py
print(f"Architecture of small_cifar_model: {small_cifar_model}")
optimizer = optim.Adam(small_cifar_model.parameters())

# Load the model state dictionary with map_location to the appropriate device
small_cifar_model.load_state_dict(torch.load(model_path, map_location=torch.device(device)))   #map_location allows to use GPU or CPU even if the model was trained on a GPU
...
small_cifar_model.to(device)

# Set the model to evaluation mode
small_cifar_model.eval()

print("The small_cifar_model and optimizer states have been loaded.")
...
with torch.no_grad():
    denoised_images_train = small_cifar_model(noisy_images_train)
...
fig.suptitle("Original, Noisy, and Denoised cifar Images of the train dataset", fontsize=16)
plt.show()
...
with torch.no_grad():
    denoised_images = small_cifar_model(noisy_images_test)
...
# Add a title to the entire figure
fig.suptitle("Original, Noisy, and Denoised cifar Images of the test dataset", fontsize=16)
plt.show()
...
with torch.no_grad():
    ...
    denoised_images_test = small_cifar_model(noisy_images_test)
    end_time = time.time()
    test_loss = criterion(denoised_images_test, images_test.to(device)).item()
    time_taken = end_time - start_time
print(f'It took {time_taken} to denoise {batchsize} images from the test dataset and the MSE loss on those images is {test_loss:.4f}')
